...
async function sendPosition(pos){
  ...
  msg(`Posizione inviata. Precisione attuale ~${Math.round(c.accuracy||0)} m. Continuo ad aggiornare per migliorare il dato.`, 'okText');
...
function start(){
  ...
  stopTimer=setTimeout(()=>{ if(watchId!==null) navigator.geolocation.clearWatch(watchId); btn.textContent='Posizione inviata'; msg(`Invio completato. Migliore precisione rilevata ~${Math.round(bestAccuracy)} m.`, 'okText'); }, 120000);
}
